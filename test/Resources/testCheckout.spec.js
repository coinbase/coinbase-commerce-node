...
describe('resources.Checkout', function () {
	...
	describe('list method', function () {
		it('should return list of checkouts', function (done) {
			...
			Checkout.list({}, function (error, response, pagination) {
				var checkoutObj = response.pop();

				assert.isArray(response);
				assert.instanceOf(checkoutObj, Checkout);
				assert.equal(checkoutObj.name, 'Test2');
	...
	describe('update method', function () {
		...
		it('should send to update only modified props', function (done) {
			...
			checkoutObj.request_info = ['skype'];

			dirtyProps = checkoutObj.getDirtyProps();

			assert.property(dirtyProps, 'name');
			assert.property(dirtyProps, 'request_info');
